...
import { getMuseSkillsDir } from "../../config.ts";

/** One bundled Muse skill: the catalog row plus where its SKILL.md body lives on disk. */
export interface MuseBundledSkill {
	/** Catalog id, e.g. `bundled:taste` or `plugin:threejs:threejs`. */
	id: string;
	/** Catalog scope, e.g. `bundled` or `plugin`. */
	scope: string;
	/** Directory under muse-skills/ that holds this skill's SKILL.md. */
	skillDir: string;
	/** Display locator from the catalog, e.g. `bundled://muse-core/skills/taste/SKILL.md`. */
	locator: string;
	/** Description text exactly as it appears inside the captured catalog's <description> element. */
	description: string;
	/** Optional short catalog description. */
	shortDescription?: string;
}

/** The 18 skills bundled with Muse Code, in captured catalog order. */
export const MUSE_BUNDLED_SKILLS: readonly MuseBundledSkill[] = [
	{
		id: "bundled:browser-app-delivery",
		scope: "bundled",
		skillDir: "browser-app-delivery",
		locator: "bundled://muse-core/skills/browser-app-delivery/SKILL.md",
		description:
	...
	{
		id: "bundled:create-skill",
		scope: "bundled",
		skillDir: "create-skill",
		locator: "bundled://muse-core/skills/create-skill/SKILL.md",
		description:
	...
	},
	{
		...
		scope: "bundled",
		skillDir: "doctor",
		locator: "bundled://muse-core/skills/doctor/SKILL.md",
		description:
	...
	{
		id: "bundled:greenfield-project-scaffolding",
		scope: "bundled",
		skillDir: "greenfield-project-scaffolding",
		locator: "bundled://muse-core/skills/greenfield-project-scaffolding/SKILL.md",
		description:
	...
	},
	{
		...
		scope: "bundled",
		skillDir: "grill",
		locator: "bundled://muse-core/skills/grill/SKILL.md",
		description:
	...
	},
	{
		...
		scope: "bundled",
		skillDir: "import",
		locator: "bundled://muse-core/skills/import/SKILL.md",
		description:
			"Resume a third-party coding-agent session from a local transcript, path, or session id. For that agent&apos;s memory notes or MCP servers, use the migrate skill instead.",
		shortDescription: "Import a Claude Code, Codex, or Grok session",
	},
	{
		id: "bundled:manage-settings",
		scope: "bundled",
		skillDir: "manage-settings",
		locator: "bundled://muse-core/skills/manage-settings/SKILL.md",
		description:
			"Any explicit Muse Code setting question or change (model, reasoning effort, /settings) requires a silent read_skill call for bundled:manage-settings as FIRST ACTION—no assistant text or other tool first. Never use for repository, app, eval, or other non-Muse configuration.",
	},
	{
		id: "bundled:migrate",
		scope: "bundled",
		skillDir: "migrate",
		locator: "bundled://muse-core/skills/migrate/SKILL.md",
		description:
			"Bring what Claude Code or Codex already remembers or has configured for this user into Muse Code — their memory notes and their MCP servers — whenever the user mentions things Claude Code or Codex remembers or knows about them, an MCP tool or server they had there that is missing here, or asks to migrate, import, or copy that setup; call read_skill for bundled:migrate before reading any foreign file. Rules and skills are already covered by /rules import and `muse skills import --from claude|codex`; point there. Do NOT use to resume or read another agent&apos;s session transcript (that is import), for Muse settings unrelated to migration (manage-settings), or when Claude Code or Codex is only mentioned in passing.",
		shortDescription: "Migrate Claude Code / Codex memory and MCP servers into Muse",
	},
	{
		id: "bundled:plan",
		scope: "bundled",
		skillDir: "plan",
		locator: "bundled://muse-core/skills/plan/SKILL.md",
		description:
			"Create a grounded, decision-complete plan, then stop for approval. Use ONLY when the user explicitly asks to plan — when they request a plan, design, approach, rollout/migration strategy, or PR breakdown, or invoke the plan skill (/skill plan). Do NOT use for ordinary implement, build, fix, debug, or refactor requests — when asked to make a code change, implement it directly without planning first. One rule applies whether or not you read the body: when an explicit plan divides work into separate diffs, commits, or PRs and you are asked to publish, publish that split — never silently collapse the planned units into fewer; tell the user before deviating.",
	},
	{
		id: "bundled:python-env",
		scope: "bundled",
		skillDir: "python-env",
		locator: "bundled://muse-core/skills/python-env/SKILL.md",
		description:
			"Setting up or installing into a Python environment. One rule applies whether or not you read the body - the environment belongs with the project, so create it inside the project directory (.venv) or let uv manage it, and never in a scratch directory like /tmp and never by forcing an install into the system interpreter. Load the body before creating a virtualenv, choosing an installer, or writing run instructions for a Python project.",
	},
	{
		id: "bundled:read-session",
		scope: "bundled",
		skillDir: "read-session",
		locator: "bundled://muse-core/skills/read-session/SKILL.md",
		description:
		...
		shortDescription: "Read this session&apos;s or a past session&apos;s logs",
	},
	{
		id: "bundled:requirements-clarification",
		scope: "bundled",
		skillDir: "requirements-clarification",
		locator: "bundled://muse-core/skills/requirements-clarification/SKILL.md",
		description:
	...
	},
	{
		...
		scope: "plugin",
		skillDir: "threejs",
		locator: "plugin://threejs/skills/threejs/SKILL.md",
		description:
			"Three.js 3D reference hub. Use when working with Three.js: pick the topic below and read its file for patterns and API guidance.",
	},
];

function stripScheme(value: string): string {
	return value.replace(/^(?:bundled|plugin):\/\//, "");
}

/** Resolve a bundled skill by id, locator path, scheme-stripped path, or skill directory. */
export function resolveMuseBundledSkill(idOrPath: string): MuseBundledSkill | undefined {
	const wanted = idOrPath.trim();
	if (!wanted) return undefined;
	const wantedStripped = stripScheme(wanted);
	for (const skill of MUSE_BUNDLED_SKILLS) {
		if (wanted === skill.id || wanted === skill.skillDir) return skill;
		if (wanted === skill.locator || wantedStripped === stripScheme(skill.locator)) return skill;
	}
	return undefined;
}

/** Read a bundled skill's SKILL.md body by id or path. Returns undefined when unknown or unreadable. */
export function readMuseBundledSkillBody(idOrPath: string): string | undefined {
	const skill = resolveMuseBundledSkill(idOrPath);
	if (!skill) return undefined;
	try {
		return readFileSync(join(getMuseSkillsDir(), skill.skillDir, "SKILL.md"), "utf-8");
	} catch {
		return undefined;
	}
}
